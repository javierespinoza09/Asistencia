...
folderA = input(  'Introdizca el path a la carpeta del archivo ')
...
for i in range(columns):
	sensoresA.append([])
	sensoresB.append([])
	sensorA_label.append(sheet.cell(0,i).value)
	sensorB_label.append(sheet.cell(0,i).value)
sensorA_label_prom = []
sensorB_label_prom = []
dates = []
...
def Read_Data():												
	for j in range(columns):										#Se busca recorrer las columnas por coordenada j
		for i in range(min([rowsA rowsB])):										#Se busca recorrer las filas por coordenada i
...
def Calc_Print():
	...
	for i in range(3):										#Se utiliza el número de columnas con el fin de conocer la cantidad de datos
		...
		sensorA_label_prom.append(sensorA_label[i+3]+' BC '+str(round(promedio[i],2))) 			#Se añade el valor del promedio del conjunto a la etiqueta con 2 decimales
		sensorB_label_prom.append(sensorB_label[i+3]+' - '+str(round(promedio[i],2)))
		ax.plot_date(dates_val, sensoresA[i+2],linestyle='solid',label=sensorA_label[i+3])			#Se realiza un plot con x=fechas,y=valor del sensor, etiqueta sensor
		ax.plot_date(dates_val, sensoresB[i+2],linestyle='solid',label=sensorB_label[i+3])
